...
namespace EManagersLib {
	public static class EPropManager {
				...
				do {
					...
					if (num7 != 0) {
						if ((num11 == x1 && num7 > 0) || (num11 == x2 && num7 < 0)) {
							startX = Max((int)((vector4.x - 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 0);
						} else {
							startX = Max(num11, 0);
						}
						if ((num11 == x1 && num7 < 0) || (num11 == x2 && num7 > 0)) {
							endX = Min((int)((vector4.x + 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 269);
						} else {
							endX = Min(num11, 269);
						}
						startZ = Max((int)((Minf(vector2.z, vector4.z) - 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 0);
						endZ = Min((int)((Maxf(vector2.z, vector4.z) + 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 269);
					} else {
						if ((num12 == z1 && num8 > 0) || (num12 == z2 && num8 < 0)) {
							startZ = Max((int)((vector4.z - 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 0);
						} else {
							startZ = Max(num12, 0);
						}
						if ((num12 == z1 && num8 < 0) || (num12 == z2 && num8 > 0)) {
							endZ = Min((int)((vector4.z + 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 269);
						} else {
							endZ = Min(num12, 269);
						}
						startX = Max((int)((Minf(vector2.x, vector4.x) - 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 0);
						endX = Min((int)((Maxf(vector2.x, vector4.x) + 72f) * (1 / PROPGRID_CELL_SIZE) + 135f), 269);
					}
					EPropInstance[] props = m_props.m_buffer;
					for (int i = startZ; i <= endZ; i++) {
						for (int j = startX; j <= endX; j++) {
		...
		private static void RefreshAutomaticProps() {
		}

		public static bool AfterDeserialize(DataSerializer s) {
			...
			for (int i = 1; i < num; i++) {
				if (buffer[i].m_flags != 0) {
					...
					if (!buffer[i].Blocked) {
						...
						district.m_parks.m_buffer[district.GetPark(buffer[i].Position)].m_propCount++;
					}
				}
			}
