...
namespace EManagersLib {
    public static class EDisasterHelpers {
        public static void DestroyProps(Vector3 position, float totalRadius, float removeRadius) {
            ...
            int startX = Max((int)((position.x - radius) * (1 / EPropManager.PROPGRID_CELL_SIZE) + 135f), 0);
            int startZ = Max((int)((position.z - radius) * (1 / EPropManager.PROPGRID_CELL_SIZE) + 135f), 0);
            int endX = Min((int)((position.x + radius) * (1 / EPropManager.PROPGRID_CELL_SIZE) + 135f), 269);
            int endZ = Min((int)((position.z + radius) * (1 / EPropManager.PROPGRID_CELL_SIZE) + 135f), 269);
